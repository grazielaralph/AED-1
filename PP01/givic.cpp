...
void bangSearchAndRemove(Stack<int>& PEP, Stack<int>& AUX, int codSearch, Queue<int>& destino){
    if(PEP.empty()){
        return;
    }

    //procura o codigo e empilha o item na aux
    while(!PEP.empty()){
        int cod = PEP.top();
        PEP.pop();
    ...
    //desempilha a aux pra empilhar na pep
    while(!AUX.empty()){
        int cod = AUX.top();
        AUX.pop();
        PEP.push(cod);
...
void bangOrganize(Queue<Comando>& FEP, Queue<int>& cancel, Queue<int>& desc, Stack<int>& PEP, Stack<int>& AUX){
    while(!FEP.empty()){
        Comando comand = *FEP.front(); 
        FEP.dequeue();                 

        switch(comand.getStatus()){
        case 'E':
            PEP.push(comand.getCod());
            break;
        case 'C':
            bangSearchAndRemove(PEP, AUX, comand.getCod(), cancel);
            break;
        case 'A':
            bangSearchAndRemove(PEP, AUX, comand.getCod(), desc);
            break;
        case '-':
            return;
        }
...
void printFEP(Queue<Comando>& queue) {
    cout << "FEP = [";
    if(!queue.empty()){
...
void printStack(const string stackName, Stack<int>& stack){
    cout << stackName << " = [";
    if(!stack.empty()){
        ListNavigator<int> stackNav = stack.getStackNavigator();
        int cod;
        while (true){
            stackNav.getCurrentItem(cod);
            cout << cod;
            if(stackNav.end()){
                break;
            }
...
int main(){
    ...
    biggyOrganize(comandos, FEP, cancel, desc);
    ...
    //Bang confere os pacotes e comandos
    bangOrganize(FEP, cancel, desc, PEP, AUX);
